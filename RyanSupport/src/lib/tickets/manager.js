...
const html_archive = require("discord-html-transcripts")
...
module.exports = class TicketManager {
	...
	async beforeRequestClose(interaction) {
		...
		this.finallyClose(ticket.id, { reason })
	}
	...
	async finallyClose(ticketId, {
		...
		if (channel?.deletable) {
			...
			//transcript code
			const attachment = await html_archive.createTranscript(channel);
			const transcriptChannel = channel?.guild.channels.cache.get("947980085762342932")
			await transcriptChannel.send({
				content: `**Ticket Information**: ${ticket.category.name} **#${ticket.number}**\n**Ticket Owner**: <@${ticket.createdById}>\n**Closed By**: ${member}`,
				files: [attachment],
			});
